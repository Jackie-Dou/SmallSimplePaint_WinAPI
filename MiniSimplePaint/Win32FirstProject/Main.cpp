...
DRAWING_OBJECTS remShapeType; //type of object
HCURSOR crossCursor;
HCURSOR handCursor;
bool zoom = false;
...
void InitOpenFileNameStructure(HWND hWnd)
{
	...
	openFileName.lpstrFilter = _T("Bitmap files(*.bmp)\0*.bmp\0JPEG files(*.jpg)\0*.jpg\0All files(*.*)\0*.*\0\0");
	openFileName.lpstrFile = new TCHAR[256];
	openFileName.lpstrFile[0] = '\0';
	openFileName.nMaxFile = 256;
	openFileName.lpstrInitialDir = _T(".\\");
	openFileName.lpstrDefExt = _T("bmp");
}
